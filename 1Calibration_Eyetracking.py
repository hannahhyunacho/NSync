# Note: This doesn't actually have eyetracking capabilities!
# this is the complement to the eyetracking nback version
# since we made some other changes to the design
...
nback_ISI = 0.75 #0.75, should translate to 0.5s
...
shuffle(animal_list_1)
shuffle(scene_list_1)
shuffle(animal_list_2)
shuffle(scene_list_2)
...
for block in range(titr_block_num):
    ...
    for thisPrac_inst_trial in prac_inst_trials:
        ...
        while continueRoutine:
            ...
            if key_resp_2.status == STARTED:
                theseKeys = event.getKeys(keyList=['space', 'r', 'f','j'])
    ...
    for thisNback_loop in nback_loop: #CHANGE TO NBACK_LOOP  IN NBACK_LOOP NO CAPS
        ...
        if target_repeat == 1: # if target is a repeat
            if key_resp_nback.keys == 'j':
                response_corr = 1 # correctly identified as target
            else:
                response_corr = 0 # otherwise incorrect
        elif target_repeat == 0: # if target is not a repeat
            if key_resp_nback.keys == 'f':
                response_corr = 1 # correctly identified as not target
            else:
                response_corr = 0 # otherwise incorrect
...
if go_to_2back:
    ...
    for block in range(titr_block_num):
        ...
        for thisNback_loop in nback_loop: #CHANGE TO NBACK_LOOP  IN NBACK_LOOP NO CAPS
            ...
            #calculates if response was correct
            if target_repeat == 1: # if target is a repeat
                if key_resp_nback.keys == 'j': # pressed j
                    response_corr = 1 # correctly identified target
                else:
                    response_corr = 0
            elif target_repeat == 0: # if target is not a repeat
                if key_resp_nback.keys == 'f': # pressed f
                    response_corr = 1 # correctly identified as not-target
                else:
                    response_corr = 0
...
real_game = visual.ImageStim(
    win=win, name='real_game',
    image='images/real_game.jpg', mask=None,
    ori=0, pos=(0, 0), size=(1200,675),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
...
key_resp_5 = event.BuilderKeyResponse()
# keep track of which components have finished
done_routineComponents = [all_done, real_game, key_resp_5]
for thisComponent in done_routineComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED
