...
justTestingPorts=False
#------------ NBACK PARAMETERS ------------ #
nback_ISI = .75 #in seconds 0.5
...
for block_count in range(len(block_order)):# FIND_ME
    ...
    while continueRoutine:
        ...
        if key_resp_2.status == STARTED:
            ...
            if "escape" in theseKeys:
                endExpNow = True
            if buttonPress != 0:  # at least one key was pressed
    ...
    for thisNback_loop in nback_loop: #CHANGE TO NBACK_LOOP  IN NBACK_LOOP NO CAPS
        ...
        # keep track of which components have finished
        nback_trialComponents = [icon, polygon, face_image, object_image, key_resp_nback,
                                 text,textTrigger, textResponse]
